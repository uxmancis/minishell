...
/*   Updated: 2024/05/11 21:21:00 by uxmancis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void get_word_infile_1(t_box **box, int *arr_ind_red_type)
{
    ...
    (*box)->infile_filenames = malloc(sizeof(char *) * tmp_nb_of_red_type);
    red_type_nb_x = 0;
    i = 0;
    while (tmp_nb_of_red_type > 0)
    {
        ...
        get_word_infile_2(arr_ind_red_type[i] + 1, (*box)->dict_red_index_type[i + 1][0] - 1, box, red_type_nb_x);
        tmp_nb_of_red_type--;
        red_type_nb_x++;
        i++;
    }
    ...
    while (tmp_nb_of_red_type > 0)
    {
        printf("                   word[%d] = "BLUE"%s\n"RESET_COLOR, i, (*box)->infile_filenames[i]);
        tmp_nb_of_red_type--;
        i++;
    }
...
void get_word_infile_2(int start, int end, t_box **box, int red_type_nb_x)
{
    ...
    (*box)->infile_filenames[red_type_nb_x] = malloc(sizeof(char) * (len_delimiter + 1));
    (*box)->infile_filenames[red_type_nb_x][len_delimiter] = '\0';
    i = 0;
    while (len_delimiter > 0)
    {
        //printf(GREEN"yepejoxepe, heredoc_nb = %d, i = %d, keep_start_word = %d\n"RESET_COLOR, heredoc_nb, i, keep_start_word);
        (*box)->infile_filenames[red_type_nb_x][i] = (*box)->input_substr[keep_start_word];
        i++;
        keep_start_word++;
        len_delimiter--;
    }
