...
/*   Updated: 2024/05/11 21:18:40 by uxmancis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void get_word_outf_app_1(t_box **box, int *arr_ind_red_type)
{
    ...
    (*box)->outfile_ap_filenames = malloc(sizeof(char *) * tmp_nb_of_red_type);
    red_type_nb_x = 0;
    i = 0;
    while (tmp_nb_of_red_type > 0)
    {
        ...
        {
            get_word_infile_2(arr_ind_red_type[i] + 2, (int)ft_strlen((*box)->input_substr) - 1, box, red_type_nb_x);
            break;
        }
    ...
    while (tmp_nb_of_red_type > 0)
    {
        printf("                   word[%d] = "BLUE"%s\n"RESET_COLOR, i, (*box)->outfile_ap_filenames[i]);
        tmp_nb_of_red_type--;
        i++;
    }
