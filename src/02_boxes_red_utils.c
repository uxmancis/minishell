...
// to check content (first word)
// after redirección.
// First, we'll copy the heredoc functions.
// Then, we'll try using them calling them both from heredoc,but
// also from other redir types.
// If successful, we'll delete previous specific functions for heredoc

/*get_nb_of_red_type
 *
 *   Gets e_red_type as input: HEREDOC / INFILE / OUTFILE_APPEND
	/ OUTFILE_STRONG
 *
 ...
 */
void	is_word_red(int **arr_word_yes_no, t_box **box, int *arr_ind_red_type,
		t_red_type red_type)
{
	int	tmp_nb_of_red_type;
	int	i;
	...
	while (tmp_nb_of_red_type > 0)
	{
		(*arr_word_yes_no)[i] = has_end_word(arr_ind_red_type[i], box,
				get_ind(arr_ind_red_type[i], box));
		tmp_nb_of_red_type--;
		i++;
	}
}
...
*                                   of redirección is asked for (e_red_type),
	the
