...
void cpy_rest_words(t_box **box)
{
    ...
    if ((*box)->nb_of_words_rest == 0)
        return;
    printf("nb_of_rest_words = %d\n", (*box)->nb_of_words_rest);
    (*box)->rest_info_potential_cmd = malloc(sizeof(char *) * ((*box)->nb_of_words_rest + 1));
    i = 0;
    while (i <= (*box)->nb_of_words_rest)
    {
        (*box)->rest_info_potential_cmd[i] = NULL;
        i++;
    }
    // (*box)->rest_info_potential_cmd[(*box)->nb_of_words_rest] = malloc(sizeof(char) * 1);
    // (*box)->rest_info_potential_cmd[(*box)->nb_of_words_rest][0] = '\0';
    //printf("done\n");
